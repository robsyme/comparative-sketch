...
function drawLinearPlot() {
  ...
  scaleFactor = Math.min(refScaleFactor,qryScaleFactor);
  ...
  .attr("transform", function(d) {return "translate(" + scalesRef[d.name](1) + ",0)";})
  .each(drawBlock)
  .each(drawLabel)
  refGroups
  .each(drawBlock)
  ...
  qryGroups.enter()
  .append("g")
  .attr("transform", function(d) {return "translate(" + scalesQry[d.name](1) + ",0)";})
  ...
  function mouseoverBlock(d, i) {
    ...
    if(this.parentElement.classList.contains("refBlockGroup")) {
      ...
      var yStart = hoff;
      var yEnd = height + hoff;
    } else {
      var xStart = woff;
      var xEnd = width + woff;
      var yStart = scaleYAccurate(d.name, positionBP) + hoff;
      var yEnd = scaleYAccurate(d.name, positionBP) + hoff;
    }
    
    selectedRegions.update(overlayCtx);
    overlayCtx.strokeStyle = "rgba(40,40,40, 0.5)";
    overlayCtx.beginPath();
    overlayCtx.moveTo(xStart, yStart);
